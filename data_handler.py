...
def To_Nifi(r, df_governorates, id, start_Gate, end_Gate, distance, start_date):
    ...
    if keys:
        for key in keys:
            ...
            if start_Gate == val[2]:
                print("This travel is recorded before.")
            else:
                ...
                print(
                    f"Violation Detection!!!! The previous key {key.decode('utf-8')} is DELETED FROM REDIS and sent as violated."
                )
        ...
        if keys_late:
            for keys in keys_late:
                ...
                print(
                    f"Delay Detection!!!! The previous key {keys.decode('utf-8').split(')')[1]} is DELETED FROM REDIS and sent as delayed."
                )
            ...
            print(
                f"A New Travel ( {id} ) Just Saved In Redis. No violation or delay detected."
            )
