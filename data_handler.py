...
def process_travels_data(df_travels, governorates_dict, r, producer):
        ...
        if ttl is not None and end_gate_code is not None:
            ...
            if last_travel_data and "ID" in last_travel_data:
                ...
                if last_travel_ttl > 0:
                    ...
                    violation_key = f"violation:{last_travel_id}"
                    violation_data = {
                        "ID": last_travel_id,
                    ...
                    print(
                        f"Adding violation for Travel ID: {last_travel_id}, End Gate: {last_travel_end_gate_code}, TTL (seconds): {last_travel_ttl:.2f}"
                    )
            ...
            r.hset(f"violation:{travel_id_with_code}", mapping=violation_data)

            # Publish the travel ID to Kafka
            kafka_message = f"Travel ID: {travel_id_with_code}, Start Date: {start_date}, TTL (seconds): {ttl:.2f}"
            producer.produce(KAFKA_TOPIC, key=travel_id_with_code, value=kafka_message)

            # Set an expiration time for the travel record key based on the calculated TTL
            travel_key = f"violation:{travel_id_with_code}"
